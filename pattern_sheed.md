...
- https://gist.github.com/haru01/6289296 のtake2

# 基本原則
* 安心感・信頼関係
* フィードバック
* 対話
* (他人は変えられないかもしれないが)自分と環境はハッキングできる (変えられる)
...
### Why
 繰り返し実行できないと、テストはやがて実行されなくなる。グリーンの状態保ちながら、機能追加、修正、リファクタリングがテストとともにプログラミング（設計）する基本となる。
...
テスト同士で依存関係を持ってしまうと、部分では通るがAllで失敗する、順番によっては失敗するなど実行結果が不安定(Erratic Test)になってしまう。Singletonや状態を保持したデータベースには注意

## Readable

### Why

テストコードもプロダクションコードも読めないと後で理解するのに困る。他人に伝えるのも困る。機能の追加修正を阻害する要因となる。よりシステムの期待する振る舞いをテストで記述するのであれば、エンジニア同士だけでなく、PO、テスターともテストを使って、コミュニケーション、コラボレーションできることが望ましい。
